 ...
 * @version $Revision: 1.93 $ ($Author: fgiust $)
 */
public class Lexer
{
...
//            else if (c == '<')  // Changed by Fran in order to properly get the script contents wheren there are '<' and '>' simbols. In these cases < must not be considered the begin of an element tag
            else if (!TidyUtils.toBoolean(esc) && c == '<')
            {
                start = this.lexsize + 1; // to first letter
                endtag = false;
                begtag = true;
            }
//            else if (!TidyUtils.toBoolean(esc) && c == '!' && lastc == '<') // Cancel start tag  // Changed by Fran in order to properly get the script contents wheren there are '<' and '>' simbols. In these cases < must not be considered the begin of an element tag
            else if (!TidyUtils.toBoolean(esc) && c == '!' && lastc == '<') // Cancel start tag
            {
                start = -1;
                endtag = false;
                begtag = false;
